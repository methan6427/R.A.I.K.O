 ...
 */
function generateMagicPacket(macAddress: string): Buffer {
 ...
 */
function parseMacAddress(mac: string): Buffer | null {
...
/**
 * Send a magic packet to wake up a device via Wake-on-LAN
...
export async function sendWakeOnLan(
  ...
  return new Promise((resolve, reject) => {
    try {
      const magicPacket = generateMagicPacket(macAddress);
